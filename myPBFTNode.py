...
class PBFTNode(myRPCProtocol):

    def __init__(self, ID=None):

        super(PBFTNode, self).__init__()
    ...
    @convert2RPC
    def clearNum(self, peer, peerId, args):
        print("clear num!!!", peer, peerId, args)
        self.Rely = 0
        self.Commit = 0;
        self.Prepare = 0;
    ...
    @convert2RPC
    def pre_prepare(self, peer, peerID, block):
        self.Prepare = 1;
        print(self.ID, " pre-prepare PBFT from ",peerID)
        peers = self.routingTable.getNeighborhoods();
        for peer_id in peers.keys():
            if peer_id != peerID:
                asyncio.ensure_future(self.prepare(peers[peer_id],self.ID,peerID,block))


    @convert2RPC
    def prepare(self, peer,peerID, PBFTID, block):

        if self.Prepare==0 :
            self.Prepare = 1
            print(self.ID, " prepare PBFT from ", PBFTID)
            #TODO check chain
            peers = self.routingTable.getNeighborhoods();
            for peer_id in peers.keys():
                if peer_id != PBFTID:
                    asyncio.ensure_future(self.prepare(peers[peer_id], self.ID, PBFTID, block))
        else:
            self.Prepare = self.Prepare+1

        print("Prepare num: ", self.Prepare, " ",self.peer_num/2+1 )

        if self.Prepare >= self.peer_num//2+1 and self.Prepare<self.peer_num:
            self.Prepare = self.peer_num
            peers = self.routingTable.getNeighborhoods()
            for peer_id in peers.keys():
                print(peer_id, "  ", peers[peer_id][0])
                if peer_id != PBFTID:
                    asyncio.ensure_future(self.commit(peers[peer_id],self.ID, PBFTID, block))

    @convert2RPC
    def commit(self, peer,peerID, PBFTID, block):

        self.Commit = self.Commit + 1
        print("commit num: ", self.Commit)

        if self.Commit >= self.peer_num // 2 + 1 and self.Commit < self.peer_num:
            self.Commit = self.peer_num
            peer = self.routingTable.getPeerById(PBFTID)
            asyncio.ensure_future(self.reply(peer, self.ID, block))

            self.blockchain.chain.append(block)
            print("Recived new Block!!!!", peer, PBFTID, block)
            logging.info("Recived new Block")
    ...
    @convert2RPC
    def reply(self, peer, peerID, block):
        self.Rely = self.Rely + 1
        print("Rely num: ", self.Rely)
        if self.Rely > self.peer_num//2 +1 and self.Rely < self.peer_num:
            self.Rely = self.peer_num
            self.blockchain.chain.append(block)
...
#收到的所有请求,更新路由表
    ...
    def handleRequest(self, peer, messageID, funcName, args, kwargs):
        peerID = args[0];
        self.routingTable.add(peerID,peer);
        super(PBFTNode, self).handleRequest(peer, messageID, funcName, args, kwargs);
    ...
    def handleReply(self, peer, messageID, response):
        ...
        self.routingTable.add(peerID,peer);
        super(PBFTNode, self).handleReply(peer, messageID, response);
    ...
    def handleBroadcast(self, peer, messageID, funcName, *args):
        ...
        if messageID not in self.BroadCasts:
            ...
            super(PBFTNode,self).handleBroadcast(peer, messageID, funcName, *args);
    ...
    def handletimeout(self, messageID, peer, args, kwargs):
        if messageID in self.requests:
            ...
            # super(PBFTNode, self).handletimeout( messageID,peer, args, kwargs);
...
# =================监听文件命令行=====================#
    ...
    async def dealCMD(self,line):
        ...
        if cmd in ["?", "help"]:
            ...
            self.mine_quick()
            print("before pop: ",len(self.blockchain.chain))
            block = self.blockchain.chain.pop()
            print("after pop: ", len(self.blockchain.chain))
            peer = ("10.0.0.1",9000)

            try:
                await self.pre_prepare(peer, self.ID, block)
                await asyncio.sleep(3)

                await self.postBoardcast(random_id(), 'clearNum', self.ID, "Testargs");
                # await self.clearNum(self.local_addr,self.ID,"Testargs");
                self.Rely = 0
            except socket.timeout:
                print(self.ID, ": could not postBoardcast")
                return
